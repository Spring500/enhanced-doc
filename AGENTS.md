...
### Bug Fix 流程（强制 Red-Green）

修复任何 bug 时必须遵循以下顺序，不可跳过：

1. **RED — 写失败测试**：根据 bug 的症状编写一个（或多个）自动化测试，运行并确认测试**失败**且失败原因精确对应 bug 症状。
2. **GREEN — 修复代码**：仅修改生产代码使测试通过，不做无关改动。
3. **验证 — 全量通过**：运行 `npm run test:all` 确认无回归。

禁止在没有失败测试的情况下直接修改生产代码。失败测试的作用：
- 证明 bug 确实存在且可复现
- 防止未来回归
- 明确修复的验收标准

**严厉禁止在修复 bug 时附带改变既有行为。** 修复的目�的仅是消除 bug 症状，不是重构、优化或调整行为。如果修复方案不可避免地需要调整行为，必须先向用户说明变更点并获得确认，绝不可自行假设"这个改动很小/很合理"而直接实施。
...
| Bug fix | 遵循上方 Bug Fix 流程（强制 Red-Green） |
